...
validMCNor_Types = function(yvec,subjectIndex,Zmat,Xmat,covTypeVec,betaI,T,Tmax,Count){
...
for (k in 1:Ktv)
{
	...
	else if(covTypeVec[k]==3) # TYPE III #
	{
		for (s in 1:T)
		{
			gEst_i[count] = Xmat[subjectIndex+s-1,k]*(yvec_i[s]-mu_i[s])
			Count[count] = Count[count]+1
			count = count + 1
		}
