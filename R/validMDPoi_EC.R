...
#' Generalized Method of Moments Valid Moment Combination Derivatives for one Subject of Longitudinal Count (Poisson) Responses, using Extended Classification
#' 
#' This function calculates the values of the derivatives of all valid moment conditions for a single subject in a longitudinal study with count (Poisson) outcomes.  It allows for unbalanced data, and uses the extended classification method to determine validity of moment conditions.  The function returns a matrix of derivatives for all valid moment condition for subject i.  
...
validMDPoi_EC = function(yvec,subjectIndex,Zmat,Xmat,betaI,T,Tmax,types){
...
dCount = 1
	...
	for(j in 1:K0)
	{
		dBetamu_i[t,dCount] = (Zmat[subjectIndex+t-1,j])*(mu_i[t])
		dCount = dCount+1
	}
	}
	...
	{
		dBetamu_i[t,dCount] = (Xmat[subjectIndex+t-1,j])*(mu_i[t])
		dCount = dCount+1
	}
}
...
## COMBINE MATRICES TO MAKE "PART" CALCULATION EASIER ##
if(!is.matrix(Zmat)){ZX = cbind(rep(1,nrow(Xmat)),Xmat)}
if(is.matrix(Zmat)){ZX = cbind(rep(1,nrow(Xmat)),Zmat,Xmat)}
...
for (t in 1:T)
{
	for (k in 1:K)
	{
		d2Betamu_i_part[t,k] = ZX[subjectIndex+t-1,k]*(mu_i[t])
	}
}
