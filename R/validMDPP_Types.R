...
validMDPP_Types = function(yvec,subjectIndex,Zmat,Xmat,covTypeVec,betaI,T,Tmax){
...
for (t in 1:T)
{
	for (k in 1:K)
	{
		d2Betamu_i_part[t,k] = (2*mu_i[t]*exp(-exp(eta_i[t]))*exp(eta_i[t])*ZX[subjectIndex+t-1,k]) + (dBetamu_i[t,k]*(1-2*mu_i[t]*exp(-exp(eta_i[t]))))
	}
}
